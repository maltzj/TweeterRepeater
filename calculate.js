...
function getTimeLocal(hours, minutes, seconds, dst)
{
  if ( (docpm) && (dochr < 12) ) {
    dochr += 12
  }
...
function calculate(timezone, year, month, day, lat, lon, dst, timeZone) {
		var jday = getJD(month, day, year);
		var tl = getTimeLocal(hours, minuts, seconds, dst);
		...
		var sunInformation = {};
		sunInformation.rise = rise;
		sunInformation.set = set;
		return toReturn;
